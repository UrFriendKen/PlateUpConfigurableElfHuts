...
namespace KitchenConfigurableElfHuts.Patches
{
    [HarmonyPatch]
    static class ChristmasPlaceSheds_Patch
    {
        static readonly Type TARGET_TYPE = typeof(ChristmasPlaceSheds);
        const bool IS_ORIGINAL_LAMBDA_BODY = false;
        const int LAMBDA_BODY_INDEX = 0;
        const string TARGET_METHOD_NAME = "OnUpdate";
        const string DESCRIPTION = "Link Christmas Sheds Reversibly"; // Logging purpose of patch
        ...
        static readonly List<object> MODIFIED_OPERANDS = new List<object>()
        {
            ...
            null,
            null,
            typeof(ChristmasPlaceSheds_Patch).GetMethod("PartnerHuts", BindingFlags.NonPublic | BindingFlags.Static)
        };

        static void PartnerHuts(ref EntityContext ctx, Entity e1, Entity e2)
        {
            ctx.Set(e1, new CConveyTeleport()
            {
                Target = e2
            });
            ctx.Set(e2, new CConveyTeleport()
            {
                Target = e1
            });
        }
